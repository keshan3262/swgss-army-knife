...
2. Start the container with PostgreSQL database using command `echo "SELECT 1;" > init-pg.sql && docker compose up -d postgres --wait --force-recreate`. It will start listening on port 20001, so if you have another process listening on it, kill it.
