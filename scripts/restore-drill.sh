...
T0=$(now_ms)
docker compose exec -T restore pg_restore -U admin -d swgss-army-knife --no-owner < "$BACKUP_DIR/swgss-army-knife.dump"
RESTORE_TIME=$(( $(now_ms) - T0 ))
AFTER=$(docker compose exec -T primary psql -U admin -d swgss-army-knife -Atc \
  "SELECT count(*) || '|' || coalesce(avg(ts), 0) FROM \
  (SELECT id, (EXTRACT(EPOCH FROM created_at) * 1000000)::bigint as ts FROM conversions)")
echo "Conversions after: $AFTER (count|avg(ts))"
if [ "$BEFORE" = "$AFTER" ]; then
  echo "Successfully restored conversions"
else
